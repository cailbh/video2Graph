...
var circlepack = Kapsule__default["default"]({
  props: {
    width: {
    ...
    nodeClassName: {},
    // Additional css classes to add on each circle node
    minCircleRadius: {
      "default": 3
    },
    excludeRoot: {
  ...
  methods: {
    ...
    _parseData: function _parseData(state) {
      if (state.data) {
        ...
        d3Hierarchy.pack().padding(state.padding).size([state.width, state.height])(hierData);
  ...
  init: function init(domNode, state) {
    ...
    var el = d3Selection.select(domNode).append('div').attr('class', 'circlepack-viz');
    state.svg = el.append('svg');
    state.canvas = state.svg.append('g'); // tooltips

    state.tooltip = el.append('div').attr('class', 'chart-tooltip circlepack-tooltip');
  ...
  update: function update(state) {
    ...
    var borderWidthOf = accessorFn__default["default"](state.borderWidth);
    ...
    var transition = d3Transition.transition().duration(animate ? state.transitionDuration : 0); // Exiting
    ...
    }).attr('r', 0).style('stroke-width', function (d) {
      return borderWidthOf(d.data);
    ...
    allCells.select('circle').transition(transition).attr('r', function (d) {
      return d.r;
    }).style('fill', function (d) {
      return colorOf(d.data, d.parent);
    }).style('stroke-width', function (d) {
      return borderWidthOf(d.data) / zoomTr.k;
    });
